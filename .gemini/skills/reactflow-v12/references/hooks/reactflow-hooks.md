...
> ⚠️ **Performance warning**: These re-render on every node move. Avoid in custom nodes. Use `useNodesData` or `useStore` with selectors instead.
...
| `nodeLookup` | `Map<string, InternalNode>` | Node lookup by ID (v12: was `nodeInternals`) |
