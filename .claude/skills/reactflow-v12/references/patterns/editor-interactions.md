# React Flow v12 — Enterprise SaaS Patterns
...
useEffect(() => { if (isUndoPressed) undo(); }, [isUndoPressed]);
useEffect(() => { if (isRedoPressed) redo(); }, [isRedoPressed]);
```
...
export function useContextMenu() {
  const [menu, setMenu] = useState<{ screen: MenuPosition; context: MenuContext }>({
    screen: null,
    context: null,
  });
  ...
  const closeMenu = useCallback(() => setMenu({ screen: null, context: null }), []);

  return { menu, onPaneContextMenu, onNodeContextMenu, onEdgeContextMenu, closeMenu };
}
...
function FlowContextMenu({ position, context, onClose }: ContextMenuProps) {
  ...
  return (
    ...
    >
      {context.type === 'pane' && (
        <>
          <ContextMenuItem icon={<Plus />} label="Add Node" onClick={() => { addNode(context.position); onClose(); }} />
          <ContextMenuItem icon={<Clipboard />} label="Paste" onClick={() => { paste(context.position); onClose(); }} />
        </>
      )}
      {context.type === 'node' && (
        <>
          <ContextMenuItem icon={<Copy />} label="Copy" onClick={() => { copy(); onClose(); }} />
          <ContextMenuItem icon={<Trash2 />} label="Delete" onClick={() => { deleteNode(context.nodeId); onClose(); }} />
          <ContextMenuItem icon={<Edit />} label="Edit" onClick={() => { openEditor(context.nodeId); onClose(); }} />
        </>
      )}
    </div>
  );
