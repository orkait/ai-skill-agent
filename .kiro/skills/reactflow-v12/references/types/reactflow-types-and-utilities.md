# React Flow v12 - Types and Utilities Reference

> Package: `@xyflow/react` (v12.x). FREE version only. All imports are named exports.
...
type Node<T = Record<string, unknown>, U = string> = {
  ...
  // Optional
  width?: number;              // Fixed width (used as inline style in v12)
  height?: number;             // Fixed height
  measured?: {                 // Read-only — set by React Flow after measuring
    width?: number;
    height?: number;
  };
  parentId?: string;           // v12: was parentNode
...
type Edge<T = Record<string, unknown>, U = string> = {
  ...
  deletable?: boolean;
  focusable?: boolean;
  reconnectable?: boolean;     // v12: was updatable
  zIndex?: number;
  ariaLabel?: string;
...
type NodeChange =
  ...
  | { type: 'add'; item: Node }
  | { type: 'replace'; id: string; item: Node };  // v12: new — replaces "reset"

type EdgeChange =
  | { type: 'select'; id: string; selected: boolean }
  | { type: 'remove'; id: string }
  | { type: 'add'; item: Edge }
  | { type: 'replace'; id: string; item: Edge };   // v12: new
```
...
type NodeProps<T extends Node = Node> = {
  ...
  zIndex: number;
  positionAbsoluteX: number;   // v12: was xPos
  positionAbsoluteY: number;   // v12: was yPos
  draggable: boolean;
  selectable: boolean;
  deletable: boolean;
  parentId?: string;
...
type OnBeforeDelete = (params: { nodes: Node[]; edges: Edge[] }) => Promise<boolean>;
