# React Flow v12 - Persistence, Theming, Testing, and Collaboration Patterns
...
function FlowKeyboardShortcuts() {
  ...
  useEffect(() => { if (isCopy) copy(); }, [isCopy]);
  useEffect(() => { if (isPaste) paste(); }, [isPaste]);
  useEffect(() => { if (isCut) cut(); }, [isCut]);
  useEffect(() => { if (isUndo) undo(); }, [isUndo]);
  useEffect(() => { if (isRedo) redo(); }, [isRedo]);
  useEffect(() => { if (isFitView) fitView({ duration: 300 }); }, [isFitView]);
...
```tsx
const restoreFlow = useCallback(async (flowId: string) => {
...
React Flow v12 uses CSS variables. Override them for custom theming:
...
test('renders flow with nodes', () => {
  ...
  expect(document.querySelector('[data-id="node-1"]')).toBeInTheDocument();
});
```
...
React Flow v12 supports server-side rendering. Pre-define node dimensions:
...
const nodes: Node[] = [
  {
    ...
    position: { x: 0, y: 0 },
    data: { label: 'Start' },
    width: 200,   // v12: used as inline style for SSR
    height: 60,
  },
];
```
